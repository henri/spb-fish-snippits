...
# one line command to configure spb-pai functioin to start multiple mistral ai a chats in multiple private browser tabs.
        ...
        while test (count $argv) -gt 0
            switch $argv[1]
                ...
                case  '--standard' '--quite' '--quiet' '--verbose' '--about'
                    echo "$argv[1]"
                case '*'
                    echo (echo "https://chat.mistral.ai/chat?q=$argv[1]" | tr " " "+")
                    set url_count (math $url_count + 1)
            end
            set --erase argv[1]
        end 
        if test $url_count -eq 0
            echo "https://chat.mistral.ai"
        end
    )
   spb $urls
end 
