# one line command which setups up spb-bit as a command to search Bitchute with provided search arguments in a private browser
        ...
        while test (count $argv) -gt 0
            switch $argv[1]
                ...
                case  '--standard' '--quite' '--quiet' '--verbose' '--about'
                    echo "$argv[1]"
                case '*'
                    echo (echo "https://www.bitchute.com/search?query=$argv[1]" | tr " " "+")
                    set url_count (math $url_count + 1)
            end
            set --erase argv[1]
        end
        if test $url_count -eq 0
            echo "https://www.bitchute.com"
        end
    )
   spb $urls
end 
...
# usage example to open private browser and search for "hello world" and "this big world" in two BitChute tabs.
# spb-bit "hello world" "this big world" 
